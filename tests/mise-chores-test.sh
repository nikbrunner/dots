...
DOTS_DIR="$REPO_DIR" PATH="$TMP_DIR/bin:$PATH" bash -c 'source "$1/scripts/dots/lib.sh"; dots_stage_mise "$DOTS_DIR" || exit 1; git -C "$DOTS_DIR" commit -qm changed; if dots_stage_mise "$DOTS_DIR"; then exit 2; fi; exit 0' bash "$ROOT" || fail 'unchanged mise files were reported as staged'
