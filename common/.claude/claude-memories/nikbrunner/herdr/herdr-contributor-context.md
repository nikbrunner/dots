...
Local setup notes: toolchain via mise (just, bun, zig). Repo requires **Zig 0.15.2** (vendored libghostty-vt); Nik had 0.16.0 installed — use `ZIG` env var or dir-scoped `mise use zig@0.15.2`. cargo-nextest was missing initially. As of 2026-07-18 the clone at `~/repos/nikbrunner/herdr` now lives at `~/repos/nikbrunner/herdr` with `origin` = nikbrunner fork, `upstream` = ogulcancelik.
