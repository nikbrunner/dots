...
**Today's picks** _(spanning {human-readable time range, e.g. "the last 8 hours" or "Feb 24–26"})_:
...
Before the table, add a single line with the time range covered by the batch, e.g. _"Feb 26, 3:00–11:00 PM"_ or _"last 4 hours"_ — derived from the `saved_at` values of the items in that batch.
...
_(You can act on multiple items at once, e.g. "later 2, 5, 8")_
