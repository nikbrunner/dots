...
- `dev:style:tanstack` -- parent skill, CLI for doc lookup
- `dev:style:state` -- server state; Form handles form state only
- `dev:style:typescript` -- type-safe patterns align with Form's inference
- `dev:style:react` -- render prop pattern for fields fits component architecture
