---
name: dev-audit-ux
description: "Audit UX quality against Jakob Nielsen's 10 Usability Heuristics — interaction design, information architecture, error handling, and user control."
...
Audit UX quality against Jakob Nielsen's 10 Usability Heuristics.

## How

- **Nielsen's 10 Heuristics** — evaluation framework applied to UI code, component tree, and interaction flows
- **`dev-util-browser`** — screenshot capture + interaction testing for visual evidence
- **LSP** — `findReferences`, `documentSymbol` for tracing error handling, navigation, and state propagation

## Steps

1. Determine scope: use argument path if provided, otherwise fall back to staged changes, then unstaged changes.
...
- `dev-audit-ui` — visual quality and a11y findings overlapping with heuristics #1, #4, #8
