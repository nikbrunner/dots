...
function M.detect_js_formatter(fname, bufnr)
    local conform = require("conform")

    local prettier_config = vim.fs.find(M.formatters.prettier.configs, { upward = true, path = fname })[1]
    local deno_config = vim.fs.find(M.formatters.deno.configs, { upward = true, path = fname })[1]
    local biome_config = vim.fs.find(M.formatters.biome.configs, { upward = true, path = fname })[1]

    local is_prettier_available = conform.get_formatter_info(M.formatters.prettier.cmd, bufnr).available
    local is_deno_available = conform.get_formatter_info(M.formatters.deno.cmd, bufnr).available
    local is_biome_available = conform.get_formatter_info(M.formatters.biome.cmd, bufnr).available
    ...
    if deno_config then
        local msg = is_deno_available and string.format("Using %s for formatting", M.formatters.deno.cmd)
    ...
    if biome_config then
        local msg = is_biome_available and string.format("Using %s for formatting", M.formatters.biome.cmd)
    ...
    elseif deno_config and is_deno_available then
        return { M.formatters.deno.cmd, stop_after_first = true }
    elseif prettier_config and is_prettier_available then
        return { M.formatters.prettier.cmd, stop_after_first = true }
    end

    return {}
end

function M.select_js_formatter()
    local bufnr = vim.api.nvim_get_current_buf()
    local fname = vim.uri_to_fname(vim.uri_from_bufnr(bufnr))
    local project_root = M.get_project_root(fname)
    ...
    local result = M.detect_js_formatter(fname, bufnr)
...
function M.init()
    ...
    conform.setup({
        ...
        formatters_by_ft = {
            javascript = M.select_js_formatter,
            javascriptreact = M.select_js_formatter,
            markdown = M.select_js_formatter,
            json = M.select_js_formatter,
            typescript = M.select_js_formatter,
            typescriptreact = M.select_js_formatter,
            css = { M.formatters.prettier.cmd },
            scss = { M.formatters.prettier.cmd },
            graphql = { M.formatters.prettier.cmd },
            html = { M.formatters.prettier.cmd },
            lua = { "stylua" },
            svelte = { M.formatters.prettier.cmd },
            yaml = { M.formatters.prettier.cmd },
            toml = { "taplo" },
            go = { "gofmt" },
            sh = { "shfmt" },
            http = { "kulala-fmt" },
