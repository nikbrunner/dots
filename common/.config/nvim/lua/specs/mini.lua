...
function M.pick()
    ...
    -- Registry: git changed files (modified + untracked)
    ...
    -- Registry: project switch with session management
    ...
    -- Registry: worktree switch
    MiniPick.registry.worktree_switch = function()
        ...
        end, vim.tbl_filter(function(wt)
            return wt.path ~= cwd
        end, worktrees))
        if #items == 0 then
    ...
    -- Registry: associated files (same base name)
    ...
    -- Registry: smart files (visited paths first, then all other files)
